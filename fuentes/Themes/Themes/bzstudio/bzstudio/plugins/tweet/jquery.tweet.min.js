...
"http://https@s3.amazonaws.com/twitter_production/default.htm"):b.profile_image_url||b.user.profile_image_url}function r(c){var e,j,h,f,a={};a.item=c;a.source=c.source;a.screen_name=c.from_user||c.user.screen_name;a.name=c.from_user_name||c.user.name;a.retweet="undefined"!=typeof c.retweeted_status;a.tweet_time=Date.parse(c.created_at.replace(/^([a-z]{3})( [a-z]{3} \d\d?)(.*)( \d{4})$/i,"$1,$2$4$3"));a.join_text="auto"==b.join_text?c.text.match(/^(@([A-Za-z0-9-_]+)) .*/i)?b.auto_join_text_reply:c.text.match(m)?b.auto_join_text_url:
