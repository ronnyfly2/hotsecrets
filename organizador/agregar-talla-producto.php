...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO relacion_tallas (rel_producto, rel_talla) VALUES (%s, %s)",
                       GetSQLValueString($_POST['rel_producto'], "int"),
                       GetSQLValueString($_POST['rel_talla'], "int"));

  mysql_select_db($database_HotSecrets, $HotSecrets);
  $Result1 = mysql_query($insertSQL, $HotSecrets) or die(mysql_error());

  $insertGoTo = "lista-tallas-producto.php?tallprod=".$_GET['tallprod'];
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_HotSecrets, $HotSecrets);
$query_DefTallasProducto = "SELECT * FROM productos_tallas ORDER BY productos_tallas.nom_talla";
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table align="center">
    <tr valign="baseline">
      ...
      <td><select name="rel_talla">
        <?php 
do {  
?>
        <option value="<?php echo $row_DefTallasProducto['id_talla']?>" ><?php echo $row_DefTallasProducto['nom_talla']?></option>
        <?php
...
?>
  ...
  <input type="hidden" name="rel_producto" value="<?php echo $_GET['tallprod'] ?>" />
